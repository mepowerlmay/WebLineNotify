...
namespace WebLineNotify.DAL
{
    ...
    public partial class LinenotifymsgDAL
    {
        ...
        /// 
        /// </summary>
        public int Add(WebLineNotify.Model.Linenotifymsg model)
        {
        ...
        /// 
        /// </summary>
        public bool Update(WebLineNotify.Model.Linenotifymsg model)
        {
            StringBuilder strSql = new StringBuilder();
        ...
        /// 
        /// </summary>
        public WebLineNotify.Model.Linenotifymsg GetModel(int id)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * from `linenotifymsg` ");
            strSql.Append(" where id=@id ");
             WebLineNotify.Model.Linenotifymsg model = null;
            using (var connection = ConnectionFactory.GetOpenConnection())
            {
                model = connection.Query<WebLineNotify.Model.Linenotifymsg>(strSql.ToString(), new { id=id }).SingleOrDefault();
        ...
        /// 
        /// </summary>
        public WebLineNotify.Model.Linenotifymsg GetModelByCond(string cond )
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * from `linenotifymsg` ");
            if (!string.IsNullOrEmpty(cond))
        ...
        WebLineNotify.Model.Linenotifymsg model = null;
            using (var connection = ConnectionFactory.GetOpenConnection())
            {
                model = connection.Query<WebLineNotify.Model.Linenotifymsg>(strSql.ToString()).SingleOrDefault();
        ...
        /// 
        /// </summary>
        public List<WebLineNotify.Model.Linenotifymsg> GetListArray(string strWhere)
        {
            StringBuilder strSql = new StringBuilder();
            ...
            List<WebLineNotify.Model.Linenotifymsg> list = new List<WebLineNotify.Model.Linenotifymsg>();
                  using (var connection = ConnectionFactory.GetOpenConnection())
            {
                list = connection.Query<WebLineNotify.Model.Linenotifymsg>(strSql.ToString()).ToList();
        ...
        /// 
        /// </summary>
        public List<WebLineNotify.Model.Linenotifymsg> GetListArray(string fileds, string orderstr, int PageSize, int PageIndex, string strWhere )
        { 

            string cond = string.IsNullOrEmpty(strWhere) ? "" : string.Format(" where {0}",strWhere);
 
			    string sql = string.Format("select {0} from `linenotifymsg` {1} order by {2} limit {3},{4}", fileds, cond, orderstr, (PageIndex - 1) * PageSize, PageSize);
          

            List<WebLineNotify.Model.Linenotifymsg> list = new List<WebLineNotify.Model.Linenotifymsg>(); 
                  using (var connection = ConnectionFactory.GetOpenConnection())
            {
                list = connection.Query<WebLineNotify.Model.Linenotifymsg>(sql).ToList();
